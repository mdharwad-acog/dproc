...
export class AutoReportGenerator {
  static async generate(
    ...
    const title =
      options.title ||
      `Analysis of ${
        bundle.metadata.source || "dataset"
      } (${recordCount} records)`;
    ...
    const prompt = this.buildPrompt(bundle, options, {
      title,
      recordCount,
      fields,
    });
    ...
    if (!result.text || result.text.trim().length === 0) {
      ...
      const fallbackContent = this.generateFallbackContent(bundle, {
        title,
        recordCount,
        fields,
      });
      ...
      log.info(
        "Fallback report generated: %d characters",
  ...
  private static buildPrompt(
    bundle: UniversalBundle,
    _options: ReportGenerateOptions,
    info: { title: string; recordCount: number; fields: string[] }
  ): string {
    const sample = bundle.samples.main.slice(0, 5);

    // Simplified prompt - let AI decide the best structure
    return `Analyze this dataset and provide a comprehensive report with insights.
...
Provide:
- High-level overview of what this dataset represents
- Key insights and patterns across the data
- Notable trends or anomalies
- Summary statistics where relevant
- Actionable recommendations based on the analysis

Write in Markdown format with clear headings and bullet points.`;
  }
  ...
  private static generateFallbackContent(
    ...
    info.fields.forEach((field) => {
      ...
      content += `- **${field}**: ${stats.type || "unknown"} (${
        stats.uniqueCount || 0
      } unique values)\n`;
    });
